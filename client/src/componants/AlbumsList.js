...
const AlbumsList = () => {
  ...
  useEffect(() => {
    dispatch(getAlbums(service.user._id));
  }, []);
  useEffect(() => {
    if (albums && albums[0]) {
          ...
          <aside className="hidden md:block md:w-72 overflow-x-hidden md:h-screen md:overflow-y-auto md:border-r-8 ">
            {albums?.map((album, i) => (
              <div
                ...
                key={i}
              >
                <img className="w-full" src={album?.album[0]} alt="" />
                <div className=" absolute text-white text-center bottom-10 rounded-xl tracking-wider w-full text-md bg-black/70 py-3">
                  <p>{album?.title}</p>
